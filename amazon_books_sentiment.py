...
reviews = reviews.withColumn(
    "book_title_clean",
    lower(trim(regexp_replace(col("book_title"), r"[^\w\s]", "")))
)
...
books = (
    books
    .withColumnRenamed("Title", "book_title")
    .withColumnRenamed("Descripe", "description")  # Note: "Descripe" typo in your dataset
...
def clean_nulls(df, columns):
    for c in columns:
        df = df.withColumn(c, when((col(c) == "") | (col(c) == "N/A") | (col(c) == "null"), None).otherwise(col(c)))
    return df
...
print(f"   Reviews with author info: {with_authors:,} ({with_authors/merged.count()*100:.1f}%)")

with_categories = merged.filter(col("categories").isNotNull()).count()
print(f"   Reviews with categories: {with_categories:,} ({with_categories/merged.count()*100:.1f}%)")

# Check author field quality
print("\n📊 Sample authors:")
merged.select("authors").filter(col("authors").isNotNull()).distinct().show(10, truncate=50)

# Check categories quality
print("\n📊 Sample categories:")
merged.select("categories").filter(col("categories").isNotNull()).distinct().show(10, truncate=50)
...
print(f"Reviews with category: {merged.filter(col('categories').isNotNull()).count():,}")
...
spark.stop()
shruti@LAPTOP-ODS225TU:~$ cat amazon_books_sentiment.py
from pyspark.sql import SparkSession
